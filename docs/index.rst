...
These docs are **literate and test-driven**: tests specify the behaviour,
doctests are the runnable examples, and the API reference is generated from the
code and tests. Narrative chapters tie them together.
...
- **Why is it built this way?** :doc:`rationale`: vision, principles, decisions
  of record.
- **Contributing?** :doc:`contributing` for the workflow, :doc:`architecture`
  for the shape, the :doc:`reference` for the code.
- **Reorienting?** :doc:`rationale` §20 (decisions) and :doc:`architecture`; the
  search box finds the rest.
