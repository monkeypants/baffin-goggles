...
diagram in :doc:`architecture`) make it so — a ``stat``\ →hash memo in the
:py:class:`~baffin.application.ports.Hasher`, a content-addressed derivative
cache keyed by :py:meth:`~baffin.domain.models.DerivativeSpec.cache_key` behind
the :py:class:`~baffin.application.ports.DerivativeStore`, and HTML that always
re-renders.
