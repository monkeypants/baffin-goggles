...
Orientation for coding agents (and humans who like a dense summary). This file
is tool-neutral; it assumes no particular assistant.
...
The domain imports nothing outward. This is enforced by `lint-imports` (an
`import-linter` contract), so a violation fails `make check` — it is not a
style preference. Keep the core (`domain`, `application`) free of I/O and
frameworks; all filesystem/pyvips/ffmpeg/HTTP work lives in `adapters`.
...
- **Commits:** Linux-kernel style — imperative summary (~50 chars), blank line,
  ...
  (`feat:`/`fix:`). No AI attribution; the human author is accountable.
