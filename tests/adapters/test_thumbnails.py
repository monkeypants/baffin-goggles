"""VipsThumbnailer: dimensions, GPS strip, embed, and the read-only original."""
...
def test_downscales_to_longest_edge(make_jpeg: JpegFactory, tmp_path: Path) -> None:
    src = make_jpeg(size=(800, 600))
    dst = tmp_path / "thumb" / "hash.jpg"
    deriv = VipsThumbnailer().render(_ref(src), THUMB, dst, strip_gps=True, embed=None)
    assert (deriv.width, deriv.height) == (300, 225)
    assert Image.open(dst).size == (300, 225)


def test_full_tier_keeps_original_dimensions(
    make_jpeg: JpegFactory, tmp_path: Path
) -> None:
    src = make_jpeg(size=(640, 480))
    dst = tmp_path / "full" / "hash.jpg"
    deriv = VipsThumbnailer().render(_ref(src), FULL, dst, strip_gps=True, embed=None)
    assert (deriv.width, deriv.height) == (640, 480)


def test_gps_is_stripped_from_output(make_jpeg: JpegFactory, tmp_path: Path) -> None:
    src = make_jpeg(
        exif={
    ...
    dst = tmp_path / "thumb" / "hash.jpg"
    VipsThumbnailer().render(_ref(src), THUMB, dst, strip_gps=True, embed=None)
...
def test_original_is_untouched(make_jpeg: JpegFactory, tmp_path: Path) -> None:
    src = make_jpeg()
    before = hashlib.sha256(src.read_bytes()).hexdigest()
    VipsThumbnailer().render(
        _ref(src), THUMB, tmp_path / "t" / "h.jpg", strip_gps=True, embed=None
    )
    assert hashlib.sha256(src.read_bytes()).hexdigest() == before


def test_embeds_authored_caption(make_jpeg: JpegFactory, tmp_path: Path) -> None:
    src = make_jpeg()
    dst = tmp_path / "thumb" / "hash.jpg"
    VipsThumbnailer().render(
        _ref(src),
